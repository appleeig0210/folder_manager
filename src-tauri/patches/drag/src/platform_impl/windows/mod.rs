        ...
        match item {
            DragItem::Files(files) => {
                ...
                unsafe {
                    ...
                    let effect = DROPEFFECT_COPY | DROPEFFECT_MOVE;

                    let dropped = if hwnd.0 != 0 {
                        match SHDoDragDrop(hwnd, &data_object, &drop_source, effect) {
                            Ok(final_effect) => final_effect != DROPEFFECT_NONE,
                            Err(_) => false,
                        }
                    } else {
                        let drop_result =
                            DoDragDrop(&data_object, &drop_source, effect, &mut out_dropeffect);
                        drop_result == DRAGDROP_S_DROP
                    };
